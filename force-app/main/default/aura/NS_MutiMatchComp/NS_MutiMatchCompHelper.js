({ // Update tab label
  ...
  /**** Updated by Sudipto G for the Static Issue */
  getAccDataInit : function(component) {
      var ANI = component.get("v.ANI");
      var formattedphone;
      var action = component.get('c.getMatchedAccountsInit');
      action.setParams({ "phone" : ANI });
      action.setCallback(this, $A.getCallback(function (response) {
          var state = response.getState();
          console.log('getAccDataInit -- NS_MultiMatchCompHelper --STATUS ' +state);
          console.log('getAccDataInit data is:' + JSON.stringify(response.getReturnValue()));
          if (state === "SUCCESS") {
              if(response.getReturnValue().accountRecords.length == 0){
				var toastEvent = $A.get("e.force:showToast");
             	toastEvent.setParams({
                     "type": "error",
                     "message": "This phone number is not stored in ConnectNS. Please search for the Account.",
