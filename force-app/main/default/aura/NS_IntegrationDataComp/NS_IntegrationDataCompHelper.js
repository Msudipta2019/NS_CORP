        ...
        action.setCallback(this, $A.getCallback(function (response) {
            ...
            if (state === "SUCCESS") {
               ...
               /* if(response.getReturnValue().GeneralInstructionElement !== undefined){
                ...
                if(response.getReturnValue().GeneralInstructionElement !== undefined){
                    	component.set('v.general_instruction_p1', response.getReturnValue().GeneralInstructionElement.GeneralInstElement);
                		 var splitString = component.get("v.general_instruction_p1");
                    
                         var listString = splitString.split('+++');
                    var gennewlst =[];
                    for(var i=0;i<listString.length -1;i++){
                        gennewlst.push(listString[i]);
                    }
                    component.set('v.generalinstlst',gennewlst); 
                    }
              /*  if(response.getReturnValue().SpecialInstructionElement !== undefined){
                ...
                if(response.getReturnValue().SpecialInstructionElement !== undefined){
                   	component.set('v.special_instruction', response.getReturnValue().SpecialInstructionElement.SpecialInstElement);
                  var splitString = component.get("v.special_instruction");
                    
                   	var listString = splitString.split('+++');
                    
                    var newlst =[];
                    for(var i=0;i<listString.length -1 ;i++){
                        newlst.push(listString[i]);
                    }
                    console.log(newlst);
                 	component.set('v.specialinstlst',newlst);       
                } 
